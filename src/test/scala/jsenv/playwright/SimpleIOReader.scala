...
final class SimpleIOReader {
      ...
      try {
        ...
        case t: TimeoutException =>
          throw new TimeoutException(
            "timed out waiting on run to call onOutputStream"
          )
      }
    ...
    try {
      ...
      case e: CancellationException =>
        throw new AssertionError(
          "unexpected exception while running read task",
          e
        )
  ...
  private def waitOnChannel(deadline: Deadline) = synchronized {
    ...
    if (_channel == null) {
      throw new AssertionError(
        "run completed and did not call onOutputStream",
        runFailureCause()
      )
    }
